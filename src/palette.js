...
{
    return engine.then(() => engineLoaded(engine));
}
...
function engineLoaded(engine)
{
    ...
    engine.sendPaletteToWorker = function()
    {
    ...
    {
        // Main browser thread only, doesn't work in a worker.
    ...
    return new Promise((resolve) => {
        // Allow this to wait for the next frame;
        // this can help avoid jerkiness in the splash animation.
        ...
        return engine.setColorTilesFromImage(require("./rewired-tileset.png"));

    }).then(() => {

        // The worker can't load images easily, send it our palette when finished.
        engine.sendPaletteToWorker();
    });
}
