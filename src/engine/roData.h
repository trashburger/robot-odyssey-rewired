...
class ROCircuit {
  public:
    struct {
        ...
        struct {
            uint8_t
                output_obj[0x100]; // 0x0000   What object does this wire drive?
        ...
        struct {
            ...
            uint8_t
                output_obj[64]; // 0x0682   What object does this wire drive?
...
class RORobot {
  public:
    ...
    ROObjectId getObjectId() { return (ROObjectId)objLeft; }
...
class RORobotBatteryAcc {
  public:
    ...
    inline uint16_t get() { return (high << 8) | low; }
} __attribute__((packed));
...
/*
 * Top level class for all data we know how to poke at in a Robot Odyssey
 * binary.
 */
class ROData {
  public:
